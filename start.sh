...
dpkg -s openjdk-7-jdk 2>/dev/null >/dev/null || apt-get -y install openjdk-7-jdk
...
java -jar selenium-grid/selenium-server-standalone-2.53.1.jar -role hub &
sleep 10
...
DISPLAY=:88 xvfb-run java -jar selenium-grid/selenium-server-standalone-2.53.1.jar -role node -port 5555 -hub http://localhost:4444/grid/register -browser browserName=firefox,platform=LINUX,maxInstances=5 &
sleep 10
